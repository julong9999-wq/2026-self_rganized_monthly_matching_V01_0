...
const AnalysisView: React.FC<Props> = ({ etfs, lastUpdated, onAddToPortfolio }) => {
  ...
  const renderDetailModal = () => {
      ...
      // 3. 判斷 AF (無配息/半年/年配)
          ...
          // 透過 AnalysisView 的 CATEGORY_MAPPING['半年'] 來判斷是否為半年配
          const semiCodes = CATEGORY_MAPPING['半年'] || [];

          if (noDivCodes.some(c => selectedEtf.code.includes(c))) {
              limit = 0;
              labelText = '無配息';
          } else if (semiCodes.some(c => selectedEtf.code.includes(c))) {
              limit = 2;
              labelText = '半年配';
          } else {
              limit = 1; // 預設年配
